...
ADJUDICATOR_MODEL = os.environ.get("SKILLGUARD_ADJUDICATOR_MODEL", "claude-sonnet-5")
...
SYSTEM_PROMPT = """You are a security adjudicator for SkillGuard, a scanner that decides \
...
def _build_prompt(findings: list[Finding], chains: list[Chain]) -> tuple[str, dict[str, Finding]]:
    ...
    lines = []
    for i, f in enumerate(findings[:MAX_FINDINGS_IN_PROMPT]):
        fid = _finding_id(f, i)
        id_map[fid] = f
        lines.append(
            ...
            f"evidence=\"{f.matched_text[:200]}\""
        )
    chain_lines = []
    for c in chains:
        chain_lines.append(
            f"- {c.chain_id}: source[{c.source.rule_id}]='{c.source.matched_text[:100]}' "
            f"(file={c.source.file}) -> sink[{c.sink.rule_id}]='{c.sink.matched_text[:100]}' "
...
def adjudicate(findings: list[Finding], chains: list[Chain]) -> str:
    ...
    try:
        ...
        for entry in parsed.get("verdicts", []):
            ...
            f.tier = _VERDICT_TO_TIER.get(verdict, Tier.INSUFFICIENT_CONTEXT)
